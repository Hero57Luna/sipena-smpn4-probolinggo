...
class Adm extends CI_Controller
{
	...
	public function kartupeserta()
	{
		...
		foreach ($data as $load) {
			...
			// ttd
			$pdf->Cell(32.5, 3, '', 0, 0);
			$pdf->Cell(20, null, $pdf->Image('upload/gambar_ttd/' . $a['lembaga']['lembaga_ttd'], null, 52, 27), 0, 1, 'C');
			$pdf->Cell(32.5, 3, '', 0, 0);
			$pdf->Cell(20, null, $pdf->Image('upload/gambar_ttd/' . $a['lembaga']['lembaga_ttd'], null, 119, 27), 0, 1, 'C');
			$pdf->Cell(32.5, 3, '', 0, 0);
			$pdf->Cell(20, null, $pdf->Image('upload/gambar_ttd/' . $a['lembaga']['lembaga_ttd'], null, 186, 27), 0, 1, 'C');
			$pdf->Cell(32.5, 3, '', 0, 0);
			$pdf->Cell(20, null, $pdf->Image('upload/gambar_ttd/' . $a['lembaga']['lembaga_ttd'], null, 253, 27), 0, 1, 'C');
			...
			$pdf->Cell(0.1, 3, '', 1, 0);
			$pdf->Cell(13.9, 3, '', 0, 0);
			$pdf->Cell(49, 3, 'Kepala Sekolah', 0, 0);
			$pdf->Cell(0.1, 3, '', 1, 1);
	...
	public function m_soal()
	{
		...
		if ($uri3 == "det") {
			...
			if (isset($_POST['order'][0]['column']) && $_POST['order'][0]['dir'] && $draw > 1) {
				$columnToSort = $_POST['order'][0]['column'];
				$columnId    = 'a.id';
				$columnJenis = 'a.jenis_soal';
				$columnSoal  = 'a.soal';
				$columnKelas = 'd.kelas';
				$columnMapel = 'c.nama';
				$columnGuru  = 'b.nama';
			...
			$q_datanya = $this->db->query("SELECT a.*, b.nama nmguru, c.nama nmmapel, d.kelas nmkelas
		                                        ...
		                                        ORDER BY $orderBy $orderDir LIMIT " . $start . ", " . $length . "")->result_array();
			$data = array();
			$no = ($start + 1);
			foreach ($q_datanya as $d) {
				$dataSoal = strip_tags(html_entity_decode($d['soal'], ENT_QUOTES));
				$dataSoal = (strlen($dataSoal) > 100) ? mb_substr($dataSoal, 0, 100) . '...' : $dataSoal;
    ...
    public function submit_all()
	{
		...
		foreach ($examIdsAnswers as $row) {
			...
			$explodedAnswerList = explode(',', $answerList);
			$jumlah_soal = count($explodedAnswerList);
			...
			$nilai = ($jumlah_benar / $jumlah_soal)  * 100;
			$nilai_bobot = ($total_bobot == 0) ? 0 : ($nilai_bobot / $total_bobot) * 100;
			$this->db->query("UPDATE tr_ikut_ujian SET jml_benar = " . $jumlah_benar . ", nilai = " . $nilai . ", nilai_bobot = " . $nilai_bobot . ", status = 'N' WHERE id = $idList");
