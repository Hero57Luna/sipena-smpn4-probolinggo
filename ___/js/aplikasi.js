...
$(document).ready(function () {
  ...
  if (url == "m_siswa") {
    ...
    paginationSoal("datatabel", base_url + "adm/m_soal/data", [{ targets: [0, 6], orderable: false }]);
    if (url2 == "edit") {
...
function paginationSoal(indentifier, url, config) {
  console.log(url);
  $("#" + indentifier).DataTable({
    language: {
      // "url": base_url+"___/plugin/datatables/Indonesian.json"
...
function submit_all() {
  var submitUrl = $("#submit_all_url").val();

  Swal.fire({
    title: 'Submit all?',
    text: "Seluruh hasil ujian peserta akan di-submit",
    ...
    preConfirm: () => {
      ...
      .then(response => {
        if (response.status == "ok") {
          Swal.fire({
            title: "Seluruh hasil peserta telah di-submit",
      ...
      .catch(err => {
        Swal.showValidationMessage('Request failed: ' + err.statusText);
      });
    }
  });
}